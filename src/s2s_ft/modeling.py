...
class BertForSequenceToSequence(BertPreTrainedForSeq2SeqModel):
    ...
    def forward(self, source_ids, noisy_source_ids, target_ids, pseudo_ids, num_source_tokens, num_noisy_source_tokens, num_target_tokens, target_span_ids=None):
        if(self.use_noisy_student >0):
            source_ids = noisy_source_ids
            num_source_tokens = num_noisy_source_tokens
...
class BertForSequenceToSequence_Distill(BertPreTrainedForSeq2SeqModel):
    def __init__(self, config):
        super(BertForSequenceToSequence_Distill, self).__init__(config)
        self.bert = BertModel(config)

        _hidden_dropout_prob = config.hidden_dropout_prob
        _attention_probs_dropout_prob = config.attention_probs_dropout_prob
        config.hidden_dropout_prob = config.teacher_dropout_prob
        config.attention_probs_dropout_prob = config.teacher_dropout_prob

        self.teacher_model = BertModel(config)

        config.hidden_dropout_prob = _hidden_dropout_prob
        config.attention_probs_dropout_prob = _attention_probs_dropout_prob

        self.use_teacher_dropout = config.use_teacher_dropout
        self.cls = BertOnlyMLMHead(config, self.bert.embeddings.word_embeddings.weight)
        self.teacher_cls = BertOnlyMLMHead(config, self.bert.embeddings.word_embeddings.weight)
        self.init_weights()
        self.kd_weight = 0
        self.log_softmax = nn.LogSoftmax()
        # setattr(config, 'label_smoothing', 0.1)
        ...
        self.use_random_replace_input = -1
        self.random_replace_input_p = -1
        self.random_replace_input_k = -1

        self.use_noisy_student = -1
        self.use_noisy_teacher = -1
    ...
    def get_kd_loss(self, student_lprobs, teacher_probs):

        # kd_loss = F.kl_div(student_lprobs.view(-1, student_lprobs.size(-1)), teacher_probs.view(-1,student_lprobs.size(-1)))
        kd_loss = F.kl_div(student_lprobs, teacher_probs, reduction='none').sum(-1)
        return kd_loss


    def forward(self, source_ids, noisy_source_ids_s, noisy_source_ids_t, target_ids, pseudo_ids, num_source_tokens, num_noisy_source_tokens_s, num_noisy_source_tokens_t, num_target_tokens, target_span_ids=None):
        if(self.use_noisy_student>0 ):
            if (self.use_random_replace_input > 0):
                with torch.no_grad():
                    input_embs = self.teacher_model.embeddings(noisy_source_ids_s)
                    top_words = torch.topk(torch.matmul(input_embs,
                                                        self.teacher_model.embeddings.word_embeddings.weight.data.transpose(
                                                            1, 0)), self.random_replace_input_k + 1, -1)[1][:, :, 1:]

                    indices = np.random.rand(*top_words.shape).argsort(axis=-1)
                    indices[indices > 1] = 0
                    noisy_words = torch.masked_select(top_words,
                                                      torch.tensor(indices).to(torch.bool).to(noisy_source_ids_s.device)).reshape(
                        top_words.shape[0], top_words.shape[1])
                    indices = torch.bernoulli(torch.ones(*noisy_words.shape) * (1 - self.random_replace_input_p)).long().to(
                        noisy_source_ids_s.device)
                    noisy_x = indices * noisy_source_ids_s + (1 - indices) * noisy_words
                noisy_source_ids_s = noisy_x
        if(self.use_noisy_teacher>0 ):
            if (self.use_random_replace_input > 0):
                with torch.no_grad():
                    input_embs = self.teacher_model.embeddings(noisy_source_ids_t)
                    top_words = torch.topk(torch.matmul(input_embs,
                                                        self.teacher_model.embeddings.word_embeddings.weight.data.transpose(
                                                            1, 0)), self.random_replace_input_k + 1, -1)[1][:, :, 1:]
                    ...
                    indices[indices > 1] = 0
                    noisy_words = torch.masked_select(top_words,
                                                      torch.tensor(indices).to(torch.bool).to(noisy_source_ids_t.device)).reshape(
                        top_words.shape[0], top_words.shape[1])
                    indices = torch.bernoulli(torch.ones(*noisy_words.shape) * (1 - self.random_replace_input_p)).long().to(
                        noisy_source_ids_t.device)
                    noisy_x = indices * noisy_source_ids_t + (1 - indices) * noisy_words
                noisy_source_ids_t = noisy_x

        if (self.use_noisy_student>0):
            student_source_ids = noisy_source_ids_s
            student_num_source_tokens = num_noisy_source_tokens_s
        else:
            student_source_ids = source_ids
            student_num_source_tokens = num_source_tokens
        if(self.use_noisy_teacher>0):
            teacher_source_ids = noisy_source_ids_t
            teacher_num_source_tokens = num_noisy_source_tokens_t
        else:
            teacher_source_ids = source_ids
            teacher_num_source_tokens = num_source_tokens

        # if (self.use_random_drop_input > 0):
        #     with torch.no_grad():
        #         indices = np.random.rand(*top_words.shape).argsort(axis=-1)
        #         indices[indices > 1] = 0
        #         noisy_words = torch.masked_select(top_words,
        #                                           torch.tensor(indices).to(torch.bool).to(source_ids.device)).reshape(
        #             top_words.shape[0], top_words.shape[1])
        #         indices = torch.bernoulli(torch.ones(*noisy_words.shape) * (1 - self.random_replace_input_p)).long().to(
        #             source_ids.device)
        #         noisy_x = indices * source_ids + (1 - indices) * noisy_words
        #     source_ids = noisy_x

        student_source_len = student_source_ids.size(1)
        teacher_source_len = teacher_source_ids.size(1)
        target_len = target_ids.size(1)
        pseudo_len = pseudo_ids.size(1)
        assert target_len == pseudo_len
        assert student_source_len > 0 and target_len > 0
        student_split_lengths = (student_source_len, target_len, pseudo_len)
        teacher_split_lengths = (teacher_source_len, target_len, pseudo_len)

        student_input_ids = torch.cat((student_source_ids, target_ids, pseudo_ids), dim=1)
        teacher_input_ids = torch.cat((teacher_source_ids, target_ids, pseudo_ids), dim=1)

        student_token_type_ids = torch.cat(
            (torch.ones_like(student_source_ids) * self.source_type_id,
             torch.ones_like(target_ids) * self.target_type_id,
             torch.ones_like(pseudo_ids) * self.target_type_id), dim=1)
        teacher_token_type_ids = torch.cat(
            (torch.ones_like(teacher_source_ids) * self.source_type_id,
             torch.ones_like(target_ids) * self.target_type_id,
             torch.ones_like(pseudo_ids) * self.target_type_id), dim=1)

        student_source_mask, student_source_position_ids = \
            self.create_mask_and_position_ids(student_num_source_tokens, student_source_len)
        student_target_mask, student_target_position_ids = \
            self.create_mask_and_position_ids(num_target_tokens, target_len, offset=student_num_source_tokens)
        teacher_source_mask, teacher_source_position_ids = \
            self.create_mask_and_position_ids(teacher_num_source_tokens, teacher_source_len)
        teacher_target_mask, teacher_target_position_ids = \
            self.create_mask_and_position_ids(num_target_tokens, target_len, offset=teacher_num_source_tokens)

        student_position_ids = torch.cat((student_source_position_ids, student_target_position_ids, student_target_position_ids), dim=1)
        teacher_position_ids = torch.cat((teacher_source_position_ids, teacher_target_position_ids, teacher_target_position_ids), dim=1)

        if target_span_ids is None:
            student_target_span_ids = student_target_position_ids
            teacher_target_span_ids = teacher_target_position_ids
        else:
            student_target_span_ids = target_span_ids
            teacher_target_span_ids = target_span_ids
        student_attention_mask = self.create_attention_mask(student_source_mask, student_target_mask, student_source_position_ids, student_target_span_ids)
        teacher_attention_mask = self.create_attention_mask(teacher_source_mask, teacher_target_mask, teacher_source_position_ids, teacher_target_span_ids)

        student_outputs = self.bert(
            student_input_ids, attention_mask=student_attention_mask, token_type_ids=student_token_type_ids,
            position_ids=student_position_ids, split_lengths=student_split_lengths)
        student_sequence_output = student_outputs[0]
        student_pseudo_sequence_output = student_sequence_output[:, student_source_len + target_len:, ]
        student_prediction_scores_masked = self.cls(student_pseudo_sequence_output)

        with torch.no_grad():
            if(self.use_teacher_dropout<0):
                self.teacher_model.eval()
                self.teacher_cls.eval()
            teacher_outputs = self.teacher_model(
                teacher_input_ids, attention_mask=teacher_attention_mask, token_type_ids=teacher_token_type_ids,
                position_ids=teacher_position_ids, split_lengths=teacher_split_lengths)
            teacher_sequence_output = teacher_outputs[0]
            teacher_pseudo_sequence_output = teacher_sequence_output[:, teacher_source_len + target_len:, ]
            teacher_prediction_scores_masked = self.teacher_cls(teacher_pseudo_sequence_output)
            teacher_prediction_scores_masked.detach_()
        ...
        kd_loss = self.get_kd_loss(F.log_softmax(student_prediction_scores_masked.float(),-1), F.softmax(teacher_prediction_scores_masked.float(),-1))
        if self.crit_mask_lm_smoothed:
            masked_lm_loss = self.crit_mask_lm_smoothed(
                F.log_softmax(student_prediction_scores_masked.float(), dim=-1), target_ids)
        else:
            masked_lm_loss = self.crit_mask_lm(
                student_prediction_scores_masked.transpose(1, 2).float(), target_ids)
        pseudo_lm_loss = loss_mask_and_normalize(
            masked_lm_loss.float(), student_target_mask)
        pseudo_kd_loss = loss_mask_and_normalize(
            kd_loss.float(), student_target_mask)

        pseudo_loss = pseudo_kd_loss * self.kd_weight + pseudo_lm_loss * (1 - self.kd_weight)
        return pseudo_loss



def get_linear_schedule_with_warmup(optimizer, num_warmup_steps, num_training_steps, last_epoch=-1, min_lr=0, fix_lr=0):
    ...
    return LambdaLR(optimizer, lr_lambda, last_epoch, min_lr, fix_lr)
...
class LambdaLR(_LRScheduler):
    ...
    def __init__(self, optimizer, lr_lambda, last_epoch=-1, min_lr=0, fix_lr=0):
